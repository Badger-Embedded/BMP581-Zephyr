...
static int get_osr_odr_press_config(struct bmp581_osr_odr_press_config* osr_odr_press_cfg, struct bmp581_data* drv);
static int set_osr_config(const struct sensor_value* osr, enum sensor_channel chan, struct bmp581_data* drv);
				      ...
				      enum sensor_channel chan)
{
	...
	if (ret == BMP5_OK)
	{
		int32_t raw_temperature = (int32_t)((uint32_t)(data[2] << 16) | (uint16_t)(data[1] << 8) | data[0]);
...
static int bmp581_channel_get(const struct device *dev,
				     enum sensor_channel chan,
				     struct sensor_value *val)
{
	...
	switch(chan)
	{
		case SENSOR_CHAN_ALTITUDE:
		{
			...
			val->val2 = ( (int32_t)(altitude * FIXED_PRECISION_COEFFICIENT) % FIXED_PRECISION_COEFFICIENT);
			return BMP5_OK;
		}
		case SENSOR_CHAN_PRESS:
